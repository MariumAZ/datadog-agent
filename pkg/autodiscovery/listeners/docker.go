...
func (l *DockerListener) createService(cID string) {
	...
	if err != nil {
		...
		if l.filters.IsExcluded(containers.Global, containerName, containerImage, "") {
	...
	if isKube {
		...
		svc = &DockerService{
			...
			checkNames:      checkNames,
			metricsExcluded: l.filters.IsExcluded(containers.Metrics, containerName, containerImage, ""),
			logsExcluded:    l.filters.IsExcluded(containers.Logs, containerName, containerImage, ""),
		}
	}
...
func (l *DockerListener) isExcluded(co types.Container) bool {
	...
	for _, name := range co.Names {
		if l.filters.IsExcluded(containers.Global, name, image, "") {
