...
func (l *KubeletListener) createService(entity string, pod *kubelet.Pod, firstRun bool) {
	...
	for _, container := range pod.Status.GetAllContainers() {
		if container.ID == svc.entity {
			// Detect AD exclusion
			if l.filters.IsExcluded(containers.Global, container.Name, container.Image, pod.Metadata.Namespace) {
			...
			svc.metricsExcluded = l.filters.IsExcluded(containers.Metrics, container.Name, container.Image, pod.Metadata.Namespace)
			svc.logsExcluded = l.filters.IsExcluded(containers.Logs, container.Name, container.Image, pod.Metadata.Namespace)
