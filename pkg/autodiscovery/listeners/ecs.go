...
func (l *ECSListener) refreshServices(firstRun bool) {
	...
	for _, c := range meta.Containers {
		...
		// Detect AD exclusion
		if l.filters.IsExcluded(containers.Global, c.DockerName, c.Image, "") {
...
func (l *ECSListener) createService(c v2.Container, firstRun bool) (ECSService, error) {
	...
	svc.metricsExcluded = l.filters.IsExcluded(containers.Global, c.DockerName, c.Image, "")
	svc.logsExcluded = l.filters.IsExcluded(containers.Global, c.DockerName, c.Image, "")
